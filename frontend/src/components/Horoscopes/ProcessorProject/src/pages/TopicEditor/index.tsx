...
import { forumImage as topicImageApi, forumTopicApi, forumTopicArchiveApi, forumTopicRestoreArchiveApi } from '../../api/forum';
...
import { buildHoroscopeUrlFromSavedHoroscope, ParsedUrlData, parseShortHoroscopeUrl } from '../../helpers/horoscopeUrl';
...
const EditTopic = () => {
    ...
    authRequest.get(forumTopicApi(id))
      .then(res => {
        ...
        setTopic({
          ...
          images_ids: [],
          members_ids: [],
          published_at: topic.published_at ? new Date(topic?.published_at) : null,
          category_name: topic.category_title,
          access_level: topic.access_level,
          category: topic.category,
          horoscope: topic.horoscope,
          horoscope_link: topic.horoscope_link,
          dim_sky: topic.dim_sky
        });
        setIsDraft(!topic.published_at || topic.status === 'archive');
        setIsArchive(topic.status === 'archive');
      })
      .finally(() => {
  ...
  const topicEditHandler = useCallback((topic: ILocalTopic) => {
    if (!isQuillEmpty(topic.description) && topic.category && topic.title) {
      dispatch(setIsAppLoading(true));
      authRequest.put(forumTopicApi(topicId), topic)
        .then(res => {
          clearTopic();
          back();
        })
  ...
  }, [clearTopic, dispatch, back, topicId]);
  ...
  const sendImage = useCallback(async (image: string) => {
    ...
    dispatch(setIsAppLoading(true));
    authRequest.post(topicImageApi(), file, {
      headers: {
  ...
  const archiveTopic = useCallback((topic: ILocalTopic) => {
    dispatch(setIsAppLoading(true));
    authRequest.post(forumTopicArchiveApi(topicId))
      .then(res => {
        back();
      })
      .finally(() => {
  ...
  }, [dispatch, back, topicId]);

  const deleteTopic = useCallback((id: number) => {
    dispatch(setIsAppLoading(true));
    authRequest.delete(forumTopicApi(id))
      .then(res => {
        back();
      })
      .finally(() => {
  ...
  const restoreArchive = useCallback(() => {
    dispatch(setIsAppLoading(true));
    authRequest.post(forumTopicRestoreArchiveApi(topicId), {
      ...topic,
      published_at: topic.published_at ? new Date(topic?.published_at) : new Date(),
  ...
  }, [dispatch, images, back, topic, topicId]);
  ...
  }, [images, isArchive, restoreArchive, topic, topicEditHandler]);

  const [isHoroscopesModalOpen, setIHoroscopesModalOpen] = useState(false);
  const [targetHoroscope, setTargetHoroscope] = useState<SavedHoroscope | ParsedUrlData>();
  const toggleHoroscopeModal = useCallback(() => {
    setIHoroscopesModalOpen((isOpen) => !isOpen);
  }, []);
  ...
  useEffect(() => {
    ...
    parseShortHoroscopeUrl(topic.horoscope_link)
      .then((data) => {
        setTargetHoroscope(data);
      });
  }, [topic.horoscope_link]);

  const horoscopeLabel = useMemo(() => {
    if (!targetHoroscope) {
      return;
    }

    if ((targetHoroscope as SavedHoroscope).id) {
      return getSavedHoroscopeLabel(targetHoroscope as SavedHoroscope);
    }

    if ((targetHoroscope as ParsedUrlData)?.address?.location?.value) {
      return `${(targetHoroscope as ParsedUrlData)?.userInfo.name}, ${(targetHoroscope as ParsedUrlData)?.address?.location?.value}`;
    } else if ((targetHoroscope as ParsedUrlData)?.address) {
      return `${(targetHoroscope as ParsedUrlData)?.userInfo.name}, Выбор на карте: ${(targetHoroscope as ParsedUrlData)?.address?.coordinates?.latitude} (Широта), ${(targetHoroscope as ParsedUrlData)?.address?.coordinates?.longitude} (Долгота)`;
    }
  }, [targetHoroscope]);

  const getButtonTextByType = useCallback((): string => {
    if (isArchive) {
      return 'Восстановить тему';
    } else if (isDraft) {
      return 'Выпустить тему';
    } else {
      return 'Сохранить тему';
    }
  }, [isArchive, isDraft]);
  ...
  return (
    <>
      ...
      <MembersListModal
        ...
        close={() => setOpenMembersModal(false)}
      />
        ...
        <Grid item container justifyContent={'space-between'} pt={2} mb={'27px'}>
          <Grid item>
            <Typography fontFamily={'Gilroy'} fontWeight={700} fontSize={'18px'} color={'#37366B'}>
              {isDraft ? 'Черновик' : 'Редактировать тему'}
            </Typography>
          </Grid>
          {/* <Grid item>
          ...
          <Grid item mb={'16px'}>
            <Grid
              ...
              <Grid flex={1} sx={{ pointerEvents: 'none' }}>
                <Input
                  value={targetHoroscope ? horoscopeLabel : ''}
      ...
      <Grid container direction={'column'} px={2}>
        ...
        <Grid item pl={6} pr={6}>
          <Button text={getButtonTextByType()} onClick={saveChanges}/>
        </Grid>
        {
          isDraft && (
            <Grid item mt={4} pl={6} pr={6} onClick={() => {
              topicEditHandler({ 
                ...topic,
                images_ids: images.map((item: any) => item.id)
              });
              ...
              <Typography
                fontFamily={'Gilroy'} fontWeight={500} fontSize={'14px'} color={'black'} textAlign={'center'}>
                Сохранить изменения
              </Typography>
            </Grid>
          )
        ...
        <Grid item pt={3} pl={6} pr={6} pb={3}>
          <Typography 
            ...
            fontFamily={'Gilroy'} fontWeight={500} fontSize={'14px'} color={'black'} textAlign={'center'}>
            {isDraft ? 'Удалить черновик' : 'Удалить тему'}
          </Typography>
        </Grid>
        <DeleteModal
          isDraft={isDraft}
          ...
          deleteTopic={() => isDraft ? deleteTopic(topicId) : archiveTopic(topic) }
        />
      </Grid>
    </>
  );

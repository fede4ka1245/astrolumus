import { FC, useMemo } from 'react';
import { Checkbox, Grid, Typography } from '@mui/material';
import Avatar from '../Avatar';
import { ITopicUser } from '../../models/interfaces/topic';
import styles from './styles.module.scss';

interface IProps {
  user: ITopicUser;
  selectedUsers?: ITopicUser[];
  onDelete?: (id: number) => void;
  onChecked?: (value: boolean) => void;  
  withCheck?: boolean;
  withDeleteButton?: boolean;
}

const UserForumPreview: FC<IProps> = ({ user, onChecked = () => {}, selectedUsers, withCheck, withDeleteButton, onDelete = () => {} }) => {
  const findedUser = useMemo(() => (withCheck && selectedUsers) ? selectedUsers.find(item => item.id === user.id) : null, [selectedUsers, user.id, withCheck]);

  return (
    <div className={styles.main} >
      <Grid container display={'flex'} alignItems={'center'} p={1}>
        <Grid item pr={2}>
          <Avatar
            width={40}
            height={40}
            fontSize={20}
            avatar={user.avatar}
            abbreviation={`${user.first_name?.slice(0, 1)}${user.last_name?.slice(0, 1)}`}
          />
        </Grid>
        <Grid item flex={1}>
          <Typography color={'#292E30'} fontWeight={500} fontSize={'14px'}>
            {user.first_name} {user.last_name}
          </Typography>
          {
            user.birth_date && (
              <Typography color={'#292E30'} fontWeight={400} fontSize={'12px'}>
                {user.birth_date.toString().split('-').reverse().join('.')}
              </Typography>
            )
          }
        </Grid>
        {withCheck && <Grid item>
          <Checkbox 
            checked={!!findedUser} 
            onChange={(e) => onChecked(e.target.checked)}
            sx={{
              '& .MuiSvgIcon-root': {
                fontSize: '25px',
                fill: '#37366B'
              },
              '& .MuiTouchRipple-root': {
                color: '#979C9E'
              }
            }}/>
        </Grid>}
        {
          withDeleteButton && (
            <Grid onClick={() => onDelete(user.id)}>
              <svg width="19" height="22" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M6.33337 8.16667V13.1667M9.66671 8.16667V13.1667M1.33337 4.83333H14.6667M13.8334 4.83333L13.1109 14.9517C13.0809 15.3722 12.8928 15.7657 12.5843 16.053C12.2758 16.3403 11.8699 16.5 11.4484 16.5H4.55171C4.13016 16.5 3.72426 16.3403 3.41578 16.053C3.10729 15.7657 2.91914 15.3722 2.88921 14.9517L2.16671 4.83333H13.8334ZM10.5 4.83333V2.33333C10.5 2.11232 10.4122 1.90036 10.256 1.74408C10.0997 1.5878 9.88772 1.5 9.66671 1.5H6.33337C6.11236 1.5 5.9004 1.5878 5.74412 1.74408C5.58784 1.90036 5.50004 2.11232 5.50004 2.33333V4.83333H10.5Z" stroke="#F15024" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </Grid>
          )
        }
      </Grid>
    </div>
  );
};

export default UserForumPreview;
